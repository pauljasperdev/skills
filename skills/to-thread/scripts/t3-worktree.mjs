...
class ToThreadError extends Error {
  constructor(code, message, details = undefined) {
    super(message);
    this.name = "ToThreadError";
    this.code = code;
...
function fail(code, message, details) {
  throw new ToThreadError(code, message, details);
}
...
async function discoverRuntime(t3Home) {
  for (const stateDirectory of ["userdata", "dev"]) {
    ...
    try {
      ...
      if (error instanceof ToThreadError) continue;
      throw error;
    }
    if (
      state?.version !== 1 ||
...
async function withSession(runtime, run) {
  ...
  try {
    ...
    if (revoked === null) {
      process.stderr.write("Warning: the temporary to-thread API session could not be revoked.\n");
    }
  }
}
...
class RpcSocket {
  constructor(socket) {
    ...
    socket.addEventListener("close", () => {
      this.closed = true;
      this.rejectAll(new ToThreadError("T3_RPC_CLOSED", "The T3 WebSocket closed unexpectedly."));
    });
    socket.addEventListener("error", () => {
      this.rejectAll(new ToThreadError("T3_RPC_FAILED", "The T3 WebSocket reported an error."));
    });
  }
  ...
  static async connect(url) {
    ...
    await new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new ToThreadError("T3_RPC_TIMEOUT", "Timed out connecting to the T3 WebSocket.")),
        10_000,
      );
      ...
      socket.addEventListener(
        ...
        () => {
          clearTimeout(timer);
          reject(new ToThreadError("T3_RPC_FAILED", "Could not connect to the T3 WebSocket."));
        },
        { once: true },
      );
    });
  ...
  async handleMessage(data) {
    ...
    try {
      ...
      this.rejectAll(
        error instanceof ToThreadError
          ? error
          : new ToThreadError("T3_RPC_PROTOCOL_ERROR", "T3 returned invalid WebSocket JSON."),
      );
      return;
    }
    ...
    for (const message of messages) {
      ...
      if (message?._tag === "ClientProtocolError") {
        this.rejectAll(
          new ToThreadError("T3_RPC_PROTOCOL_ERROR", "T3 rejected the WebSocket RPC protocol.", {
      ...
      if (message?._tag === "Defect") {
        this.rejectAll(
          new ToThreadError("T3_RPC_DEFECT", "T3 reported a WebSocket RPC defect.", {
      ...
      if (message.exit?._tag === "Success") {
        ...
        pending.reject(
          new ToThreadError("T3_RPC_COMMAND_FAILED", `T3 rejected RPC ${pending.tag}.`, {
  ...
  call(tag, payload, timeoutMs = DEFAULT_TIMEOUT_MS) {
    if (this.closed || this.socket.readyState !== globalThis.WebSocket.OPEN) {
      return Promise.reject(new ToThreadError("T3_RPC_CLOSED", "The T3 WebSocket is not open."));
    }
    const id = randomUUID();
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(new ToThreadError("T3_RPC_TIMEOUT", `Timed out waiting for RPC ${tag}.`));
      }, timeoutMs);
...
function branchFragment(label) {
  const normalized = label
    ...
    .replace(/-+/g, "-")
    .replace(/^[._-]+|[._-]+$/g, "");
  return normalized.slice(0, 64).replace(/[._-]+$/g, "") || "task";
}

async function resolveWorktreeBranch(cwd, label) {
  const fragment = branchFragment(label);
...
function validateThreadSpec(spec) {
  if (!spec || typeof spec !== "object" || Array.isArray(spec)) {
    fail("INPUT_INVALID", "open --json expects one JSON object on stdin.");
  }
  if (typeof spec.title !== "string" || !spec.title.trim() || /[\r\n]/.test(spec.title)) {
    fail("TITLE_INVALID", "title must be a non-empty single line.");
  }
  if (typeof spec.prompt !== "string" || !spec.prompt.trim()) {
    fail("PROMPT_INVALID", "prompt must be a non-empty first task.");
  }
  for (const [field, code] of [["branchLabel", "BRANCH_LABEL_INVALID"], ["dedupeKey", "DEDUPE_KEY_INVALID"]]) {
...
async function verifyCreated(runtime, token, expected, project) {
  ...
  while (Date.now() < deadline) {
    ...
    if (thread.latestTurn?.state === "error") {
      fail("T3_TURN_FAILED", "The first T3 turn failed to start.", {
        threadId: expected.threadId,
    ...
    try {
      ...
      if (error instanceof ToThreadError) throw error;
    }
...
export async function runCli(open = openThread, inputExample = '{"cwd":"/repo","title":"Task title","prompt":"First task"}') {
  try {
    ...
    const normalized =
      error instanceof ToThreadError
        ? error
        : new ToThreadError(error?.code ?? "UNEXPECTED", error instanceof Error ? error.message : "Unexpected failure.");
